...
class AccountingService:
    ...
    def add_payment(
        ...
        month_balance = self.movements.get_month_balance(student_id, month, year)
        ...
        movement = self.movements.add(movement=movement)
        new_balance = self.movements.get_balance(student_id)
    ...
    def add_fee(self, month: int, year: int) -> int:
        ...
        try:
            self.movements.apply_fees(data)
    ...
    def reverse(self, payment_id: int) -> Movement:
        ...
        if self.movements.has_reversal(orig.id):
        ...
        movement = self.movements.add(movement)
        return movement
    ...
    def get_unpaid_months_with_debt(
        ...
        passed_months = self.movements.get_general_month_balance(student_id)

        return [month for month in passed_months if month[2] < 0]

    def get_all_movements(self) -> list[Movement]:
        return self.movements.get_all()

    def get_effective_payments(self) -> list[Movement]:
        return self.movements.get_effective_payments()

    def get_effective_fees(self) -> list[Movement]:
        return self.movements.get_effective_fees()

    def get_effective_movements_by_id(self, student_id: int):
        return self.movements.get_effective_movements_by_id(student_id)

    def get_balance_by_id(self, student_id: int) -> int:
        return self.movements.get_balance(student_id)
    ...
    def get_students_without_fee(self, month: int, year: int) -> list[Student]:
        return self.students.get_students_without_fee(month, year)
    ...
    def get_overview(self, student_id: int) -> StudentOverview:
        return StudentOverview(
            student=self.students.get_by_id(student_id),
            balance=self.movements.get_balance(student_id),
